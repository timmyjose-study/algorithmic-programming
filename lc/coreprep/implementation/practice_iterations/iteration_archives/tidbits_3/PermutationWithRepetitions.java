...
public class PermutationWithRepetitions {
  ...
  private static List<List<Integer>> permute(int[] a) {
    ...
    permute(a, 0, a.length, visited, currPerm, perms);

    return perms;
  }

  private static void permute(int[] a, int currIdx, int n, boolean[] visited,
    ...
    for (int i = 0; i < n; i++) {
      ...
      visited[i] = true;
      currPerm.add(a[i]);
      permute(a, currIdx + 1, n, visited, currPerm, perms);
      visited[i] = false;

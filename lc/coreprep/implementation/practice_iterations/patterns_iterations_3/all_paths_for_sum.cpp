...
void all_paths_with_sum(TreeNode *root, int sum, vector<int> &curr_path,
  ...
  if (root->left == nullptr && root->right == nullptr && sum == 0) {
    ...
    all_paths_with_sum(root->left, sum, curr_path, all_paths);
    all_paths_with_sum(root->right, sum, curr_path, all_paths);
  }
...
vector<vector<int>> all_paths_with_sum(TreeNode *root, int sum) {
  ...
  all_paths_with_sum(root, sum, curr_path, all_paths);

...
int min_diff(const vector<int> &a, int sum1, int sum, vector<vector<int>> &dp,
             int curr_idx) {
  if (curr_idx == a.size()) {
    int sum2 = sum - sum1;
  ...
  if (dp[curr_idx][sum1] == -1) {
    int diff1 = min_diff(a, sum1 + a[curr_idx], sum, dp, curr_idx + 1);
    int diff2 = min_diff(a, sum1, sum, dp, curr_idx + 1);

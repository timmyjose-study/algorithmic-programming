...
int coin_change_ways(int amount, const vector<int> &coins,
                     vector<vector<int>> &dp, int curr_idx) {
  if (amount == 0) {
    return 1;
  }
  ...
  if (dp[curr_idx][amount] == -1) {
    ...
    if (coins[curr_idx] <= amount) {
      ways1 = coin_change_ways(amount - coins[curr_idx], coins, dp, curr_idx);
    }

    int ways2 = coin_change_ways(amount, coins, dp, curr_idx + 1);
...
int coin_change_ways(int amount, const vector<int> &coins) {
  ...
  return coin_change_ways(amount, coins, dp, 0);
}

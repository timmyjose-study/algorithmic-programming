...
int lpss(const string &s, int start_idx, int end_idx, vector<vector<int>> &dp) {
  ...
  if (dp[start_idx][end_idx] == -1) {
    if (s[start_idx] == s[end_idx]) {
      ...
      if (rem_len == lpss(s, start_idx + 1, end_idx - 1, dp)) {
        dp[start_idx][end_idx] = 2 + rem_len;
        return dp[start_idx][end_idx];
      }
    }

    int len1 = lpss(s, start_idx + 1, end_idx, dp);
    int len2 = lpss(s, start_idx, end_idx - 1, dp);
    dp[start_idx][end_idx] = max(len1, len2);
  }
